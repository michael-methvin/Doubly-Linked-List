...
/*here*/	newNode.previous = tail;	// **** add new codes here to handle the head.previous ****
...
/*here*/	newNode.previous = tail;
	...
	public E remove(int index) {	/** add code to set 'previous' */
		if (index < 0 || index >= size)
			...
			for (int i = 1; i < index; i++)
				previous = previous.next; // getNodeAt update first
			Node<E> current1 = previous.next;
	...
	public E set(int index, E e) {	/** improve performance using 'previous' */
		if (index < 0 || index > size - 1)
			return null;
		Node<E> current = getNodeAt(index);   // ** need to replace this with getNodeAt(index) and the following for loop
